...
const UserSearch: React.FC<UserSearchProps> = ({ isSearchList, setQuery, papers }) => {
	...
	return (
		<div className="lf-plugin-name">
			<div className="lf-search-form">
				<div className="index-search">
					{!isCSL &&
						<div className='lf-plugin-global-copy-disabled'>
							<CopyIcon />
						</div>
					}
					...
					<input
						type="search"
						className={`lf-search-input ${isSearchList ? 'lf-index-search' : 'lf-index-no-search'}`}
						placeholder="Literature Flow"
						onChange={(e) => {
							if (!setQuery) return;
...
export const ReferenceMapList = (props: {
	...
	if (papers.length > 0) {
		return (
			<>
				<div className="lf-literature-flow">
					<UserSearch
						isSearchList={true}
						setQuery={setQuery}
						papers={papers}
					...
					{indexSearch(papers, query).map((paper, index) => {
						...
						const activeIndexCardClass = selection?.includes(paperId) ? 'lf-active-index' : '';
